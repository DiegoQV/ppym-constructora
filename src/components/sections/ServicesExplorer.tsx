...
export function ServicesExplorer({ services, phone }: ServicesExplorerProps) {
  ...
  return (
    <div className="border-t border-charcoal/18" aria-busy={transitionPhase !== "idle"}>
        ...
        return (
          <article key={service.id} className="border-b border-charcoal/18">
            <button
              ...
              className={cn(
                "group relative grid w-full grid-cols-[2.75rem_1fr_auto] items-center gap-3 overflow-hidden border-l-2 px-3 py-5 text-left transition-[color,border-color,background-color] duration-300 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-technical-yellow sm:grid-cols-[3.5rem_1fr_auto] sm:px-4 sm:py-6",
                isActive ? "cursor-default border-l-technical-yellow bg-[#f6f4e8] text-charcoal lg:sticky lg:top-24 lg:z-20 lg:shadow-[0_10px_22px_rgba(17,19,21,.06)]" : "border-l-transparent text-charcoal/68 hover:bg-charcoal/[.025] hover:text-charcoal",
              )}
            >
              ...
              <span className={cn("font-heading text-xl font-semibold tracking-[-0.025em] transition-transform duration-300 ease-out sm:text-2xl lg:text-[1.7rem]", !isActive && "group-hover:translate-x-1")}>
                {service.title}
              </span>
            ...
            <div id={panelId} hidden={!isActive}>
              <div className={cn("relative mb-5 overflow-hidden bg-charcoal text-white sm:mb-6", transitionPhase === "exiting" ? "service-panel-exit" : "service-panel-enter")}>
                ...
                <div className="relative px-5 py-6 sm:px-7 sm:py-8 lg:grid lg:grid-cols-[0.85fr_1.15fr] lg:gap-10 lg:px-9 lg:py-9">
                ...
                <div className="service-copy-enter relative">
                  <p className="max-w-md text-sm leading-6 text-white/74 sm:text-base sm:leading-7">
                    {service.description}
                  </p>
                  <a
